...
final class SelectionUserReviewLimitInlineKeyboard extends InlineKeyboard
{
    ...
    {
        return array_map(
            fn(int $limit) => $this->makeButton(ReviewLimit::fromInt($limit)),
            self::LIMIT_LIST,
        );
    }

    private function makeButton(ReviewLimit $limit): InlineKeyboardButton
    {
        return InlineKeyboardButton::make(
            text: (string)$limit->value(),
            callback_data: SettingsMenuCommand::SET_REVIEW_LIMIT_CALLBACK->value . $limit->value(),
        );
    }
}