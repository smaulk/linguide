...
final class CheckReviewAnswerAction extends Action
{
    /**
    ...
    public function run(TermVariantDto $termVariant, string $answer): ReviewAnswerResult
    {
        if ($termVariant->translations === []) {
            throw new LogicException(
                "Вариант {$termVariant->id} термина '{$termVariant->text}' не имеет переводов."
            );
        }
    ...
    private function isAnswerCorrect(string $translation, string $answer): bool
    {
        ...
        $tolerance = match (true) {
            $valueLen <= 4  => 0, // короткие — только точное совпадение
            $valueLen <= 7  => 1, // до 7 символов — 1 ошибка
            $valueLen <= 11 => 2, // до 11 символов — 2 ошибки
            default         => 3, // длинные — до 3 ошибок
        };
